...
<div class="flex-grow p-12 bg-arena-landscape-blurred bg-cover bg-fixed">
	...
	<div class="flex flex-col items-center lg:flex-row lg:items-stretch justify-center">
		...
		{:else if myGames}
			<div
				class="flex flex-col justify-center items-center text-center p-8 m-8 rounded-xl bg-white bg-opacity-50 drop-shadow-lg"
			>
				...
				<table class="mx-auto border border-slate-300">
					...
					{#if myGames.length > 0}
						{#each myGames as game}
							{@const opponents = game.gamePlayers?.filter(
								(gp) => gp.player.minaPublicKey !== player1
							)}
							<tr class="[&>*]:p-2 [&>*]:text-center border-b border-slate-300">
								<td class="text-red-600"
									>{game.currentPhase?.gamePlayer.player.minaPublicKey === player1
										? 'Your Turn!'
										: ''}</td
								>
		...
		<div
			class="flex flex-col justify-center items-center text-center lg:w-1/4 max-w-md p-8 m-8 rounded-xl cursor-pointer bg-white hover:bg-stone-300 bg-opacity-50 drop-shadow-lg"
		>
