<script lang="ts">
  ...
  export let pieceHoveredHandler: (piece: GamePiece, mouseAbsolutePoint: Point) => void;
  export let pieceUnhoveredHandler: () => void;
  export let rerender: () => {};
  ...
  const drawMovementArrow = (
    ...
    Utils.drawArrow(ctx, moveAction.action.moveFrom, moveAction.action.moveTo);
  }
